...
<br>
[R](http://www.r-project.org/) is an extremely powerful programming language for working with datasets, applying statistics and creating publication ready graphics. In this lesson you will learn how to program in R and use the RStudio server environment. We will cover the basics of the R syntax using built in packages (base R), as well as importing data, creating simple graphics and running statistical tests. We will also introduce some of the extended libraries that make R the first choice for analysing genomic data.
...
* **Free!**
* **Powerful**, many libraries have been created to perform application specific tasks. e.g. Next-Gen sequencing (bioconductor).
...
* History
    ...
    + Create automated scripts to perform again and again on different data.
* [RStudio](https://www.rstudio.com/) provides an interactive environment for workin in R.
...
* **Vector**  
    + a list of values, (equivalent to a column in a table)
* **Data Frame**
...
Download and save the tutorial data sets [data](http://bifx-core.bio.ed.ac.uk/data.tsv) and  [weight](http://bifx-core.bio.ed.ac.uk/weight.tsv) into your working directory. 
...
Example storing output of summary function ('sum_cars' could be named whatever you like)
```{r}
sum_cars = summary(cars)
```

To see result we need to type:
```{r}
sum_cars
```
...
Even quit is a function (although, you can quit using the RStudio interface). Note I have commented this out using '#' so I do not accidently run this command!
...
**NOTE** quotes are needed for search strings, they are not needed when referring to data objects name or a function.
...
For a beginner, this can be the hardest part, it is also the most important to get right. 
...
You can also use the `View()` command to open data frames in the files pane.
...
Use `fill=TRUE` if there is missing data
```{r}
...
In the *Envirnoment* pane in RStudio there is a button called "Import Dataset". This can make importing data much easier and calls the `read.*` set of functions for you. The command used will be displayed on the console. Note that you need to have the file on the computer to use this button.
...
We recommend learning **`ggplot2`** for graphics but it is useful to know the options available in "base" R.

Remember, to get more information about the options available to a function, type '?function'
...
If there was more data, like in this large collection of Chick weights, we can increase the number of vertical columns with the option, `breaks=50` (or another relevant number). 
...
Note that nothing will appear on screen, the output is going to the file, 'dev.off()' here stops device and saves the file.

## Statistical testing

R has many functions for statistical testing. Please see our [introduction to stats](https://drive.google.com/open?id=1qG_4Dzxc9x3yDnkhjInFKSUGdEHmn2shW2Sj6KwSc_A) slides for more information on distributions, hypothesis testing and statistical significance. 
...
The Shapiro Wilk test is a non-parametric test. So, a significant p-value (p < 0.05) means that the data is NON-parametric. As p here = 0.3088 (> 0.05), A is parametric.
...
> ## Key Points
>
> **IMPORTANT:** You should only use parametric tests for parametric data!
{: .keypoints}

### T-Test

The assumption for a t-test is that both groups are sampled from normal distributions with equal variances, i.e. we can only use a parametric test if the data is normally distributed. As columns A and E are both normally distributed we can use the Welch Two Sample t-test a *t-test* to test if the mean values are statistically different. Our null hypothesis is that the two means are equal, and the alternative is that they are not.
...
Load the paired dataset, 'weight.tsv', which contains weights of mice before and after treatment X.

> ## Challenge 5
>
> See if you can load the mice weight data. The file is here: **http://bifx-core.bio.ed.ac.uk/weight.tsv** .
> Save this as an object called `weight`, check you have read it correctly and try plotting it.
>
> > ## Solution
> >
> >```{r}
...
```{r}
shapiro.test(weight$diff) # Or shapiro.test(weight$diff)
```

Looks good, the p-value is greater than 0.05 implying that the distribution of the differences (d) are not significantly different from normal distribution. In other words, we can assume normality.
...
A matrix is a data frame where the values in rows are related across columns. You can turn a data frame into a matrix using the `as.matrix()` function.
